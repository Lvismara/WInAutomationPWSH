...
$hostName = $Pcname+$((Get-Date).ToString('MM-dd-yyyy'))
...
powershell -ExecutionPolicy ByPass -File ListAllSharedFolderPermission.ps1 | Out-File ./$hostName/$hostName+_Paths$((Get-Date).ToString('MM-dd-yyyy')).csv
